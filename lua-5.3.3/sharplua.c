...
#else
typedef int(__stdcall *csharp_callback)(int argc, struct var *argv, struct string_pusher *sp);
#endif
...
Lsharpcall(lua_State *L) {
	...
	cb = (csharp_callback)lua_touserdata(L, lua_upvalueindex(1));
	
	n = lua_gettop(L);
	ok = marshal_vars(L, n , arg);
	if (ok == -1) {
...
Lgetfunction(lua_State *L) {
	...
	t = lua_getglobal(L, (const char *)key);
	if (t == LUA_TNIL) {
		lua_pushinteger(L, 0);
		return 1;
	}
...
c_callfunction(lua_State *L, int argc, struct var *argv, int strc, const char **strs) {
	...
	retn = lua_gettop(L) - top;
	result = marshal_vars(L, retn, argv);
